...
export class HwpDocument {
     ...
     * 증분 페이징을 한 단계 진행한다.
     */
    stepProgressivePaging(chunk_size: number): number;
    /**
     * 텍스트 오프셋 → 논리적 오프셋 변환.
     */
...
export interface InitOutput {
    ...
    readonly hwpdocument_isPagingFinished: (a: number) => number;
    ...
    readonly hwpdocument_startProgressivePaging: (a: number) => [number, number];
    readonly hwpdocument_stepProgressivePaging: (a: number, b: number) => [number, number, number];
