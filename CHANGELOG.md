...
> 성능 혁신 — "대규모 문서의 부드러운 로딩과 지연 페이징"

### 성능 최적화 (Project Phoenix)
- **증분 페이징 (Incremental Pagination)**: Rust 엔진을 Resume 가능한 상태 머신으로 개편하여 수천 페이지 문서도 백그라운드에서 끊김 없이 처리. (#176)
- **Async 로딩 파이프라인**: `requestIdleCallback`과 WASM 증분 API를 연동하여 첫 페이지 노출 시간을 기존 대비 90% 이상 단축. (#177)
- **GPU 메모리 관리**: `CanvasPool` 도입으로 활성 캔버스 개수를 엄격히 제한하여 대용량 문서 열람 시의 브라우저 메모리 릭 및 속도 저하 문제 완전 해결. (#178)
- **가상 스크롤 가속**: `requestAnimationFrame` 스로틀링 및 뷰포트 관리 최적화를 통해 대규모 문서에서도 60FPS에 부합하는 부드러운 스크롤 구현. (#179)

### 보안 및 정책
- **로컬 보안 정책 명시**: "모든 문서 처리는 로컬 시스템 내에서만 수행됨"을 제품 정보 및 보안 정책 문서에 명시하여 사내 보안 규정 준수 보장.
- **GitHub Pages 배포 최적화**: 루트 랜딩 페이지와 에디터 `/editor/` 경로 분리 처리를 통해 웹 접근성 및 CI/CD 안정성 개선.

### 수정
- **WASM 엔진 초기화 랙**: 문서 로드 시의 동기적 `paginate()` 호출을 제거하고 비동기 제어권 반환 로직 적용.
- **제품 정보 문구**: 사내 보안 정책 안내 문구를 간결하고 명확하게 수정.
